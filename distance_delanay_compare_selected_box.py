...
def onselect(eclick, erelease):
    ...
    if len(unique_classes) == 1:
        ...
        print(f"\n Selected {len(selected)} points from Class {unique_classes} for Box {box_id}")
...
def update_distance_matrix(data, selected_points_flat,labels , bNorm_mean = False):
    """
...
#         ####________________ Add label text_______________________

#         # Compute center positions
#         center_x = (start + end) / 2
#         # center_y = (start + end) / 2

#         # Add label text at top-left corner of each box
#         label = f'Box {i+1}' 
#         ax.text(center_x, -0.5, label, color=color, fontsize=8, ha='center', va='bottom', clip_on=False)

#         ax.text(-1.0, center_x, label,
...
def visualize_distance_matrix(dist_matrix, labels, mean_block_distance, box_colors=None, figsize=(10, 10), output_path=None):
    ...
    Args:
        dist_matrix: (N x N) symmetric matrix of distances
        labels: list of box IDs for each row/col in dist_matrix
        mean_block_distance: (K x K) matrix of mean distances between clusters
        box_colors: optional list of colors for each cluster
        figsize: size of the figure
    ...
    #     colors = [box_colors[i] for i in unique_boxes]
    # else:
    #     # Use default color cycle if no box_colors provided
    #     colors = plt.cm.tab10.colors

    # For dynamic label alignment
    max_pos = cumulative_positions[-1]
    text_offset_top = max_pos * 0.06  # small offset for text above plot
    text_offset_beside = max_pos * 0.08  # small offset for text beside plot
    ...
    for i in range(len(unique_boxes)):
        ...
        # Left border (outer vertical)
        ax.plot([-0.5, -0.5], [start - 0.5, end - 0.5], color=color, linewidth=border_thickness, solid_capstyle='butt', clip_on=False)

        # Top border (outer horizontal)
        ax.plot([start - 0.5, end - 0.5], [-0.5, -0.5], color=color, linewidth=border_thickness, solid_capstyle='butt', clip_on=False)

        # Top label (center aligned above)
        ax.text(center, -text_offset_top, f'Box {i+1}', color='black', fontsize=8, ha='center', va='bottom', clip_on=False)

        # Left label (center aligned vertically, rotated)
        ax.text(-text_offset_beside, center, f'Box {i+1}', color= 'black', fontsize=8,
    ...
    for i, start_i in enumerate(cumulative_positions[:-1]):
        ...
        for j, start_j in enumerate(cumulative_positions[:-1]):
            ...
            center_x = (start_j + cumulative_positions[j + 1]) / 2
            center_y = (start_i + cumulative_positions[i + 1]) / 2
            ax.text(center_x, center_y, f"{mean_distance:.2f}", color="black", ha="center", va="center", fontsize=6,
                    bbox=dict(boxstyle="round", facecolor="white", edgecolor="0.3", alpha=0.6))
...
def create_display_edges_between_selected_boxes():
    ...
    dist_matrix, labels, point_indices,  dist_matrix_mean = update_distance_matrix(D, selected_points_flat, labels, bNorm_mean = True)
    ...
    ###______________________________Sub delaunay_______________________________
    ...
    print('run barycentric coordinate interpolation')

    # # # sub_intensity_interp_cordinates, x_min, x_max, y_min, y_max = barycentric_interpolation_coordiantes(n_grid_points_inter, sub_orig_data, sub_low_dm_emb, sub_tri_delaunay, inverse_model.eval(), blog = False, bclamping = False)
    sub_intensity_interp_cordinates, x_min, x_max, y_min, y_max = barycentric_interpolation_coordiantes_batch(n_grid_points_inter, sub_orig_data, sub_low_dm_emb, sub_tri_delaunay, inverse_model.eval(), blog = False, bclamping = False)    
    output_path = os.path.join(output_folder, f"barycentric_sub_coordianates_interpolation_{method_name}_{perplexity}_{dataset}_{method}")
    plot_interpolation(n_class, sub_low_dm_emb, labels, colors, sub_intensity_interp_cordinates, x_min, x_max, y_min, y_max, bscatter_plot = True, background_color =  'white', output_path=output_path, figsize=figsize)

    print('Run fully connected points blending methods ...')
    output_path = os.path.join(output_folder, f"Fully_sub_connected_clusters_point_blending_new_{method_name}_{perplexity}_{dataset}_{method}")
    plot_fully_connected_points_optimized(sub_low_dm_emb, sub_orig_data, labels, n_class, colors, blend_methods=['average', 'max', 'min'], bscatter_plot = True, output_path=output_path, image_size=(500, 500), figsize=(10,10))
