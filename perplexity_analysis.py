...
####_______________plotting___________________
...
#     D, c, centers = generate_high_dimension_gaussians(num_dim = args.num_dim, n_pts_per_gauss=200, spread_factor= spread_factor, distance_factor=distance_factor, distance_factor_2= distance_factor_2, move_cluster_index=move_cluster_index)
#     dim = D.shape[1]
#     output_size = dim
#     n_gauss = len(np.unique(c))
...
#     method=  f'_df_{distance_factor}_df_2_{distance_factor_2}'
...
#     method = f'_cl_spac_{cluster_spacing}'
...
#     method = f'_high_dim_{dim}'
    
# else:
#     # output_folder = f"thesis_reproduced/testing_new/perplexity_analysis/{dataset}/{method}_plots_new_model"
#     output_folder = f"thesis_reproduced/testing_new/pca_analysis_har/{dataset}/{method}_plots_new_model"
#     method = f'{dataset}'
...
# breakpoint()
# output_path = os.path.join(output_folder, f"HD_clust_distance_{dataset}_{method}")
...
# output_path = os.path.join(output_folder, f"HD_clust_mean_distance{dataset}_{method}")
...
for i, perplexity in enumerate(perplexities):
    ...
    # output_path = os.path.join(output_folder, f"Fully_connected_clusters_point_edges_{method_name}_{perplexity}_{dataset}_{method}")
    # print('Start plotting fully edges graphs ...')
    # plot_fully_connected_points_edges(low_dm_emb, D, c,n_gauss, point_color=colors, edge_sorts = ['normal', 'ascending', 'descending'], figsize = figsize, output_path=output_path)
    
    print('Start plotting fully edges graphs with blending ...')
    output_path = os.path.join(output_folder, f"Fully_connected_clusters_point_blending_{method_name}_{perplexity}_{dataset}_{method}")
    ...
    # ### output_path = os.path.join(output_folder, f"delaunay_tri_max_rel_lenth_histogram_{method_name}_{perplexity}_{dataset}_{method}")
    # ### plot_histogram_values(max_rel_edge_ratio_per_row, bins= 30, output_path=output_path)
    # ### output_path = os.path.join(output_folder, f"delaunay_tri_max_rel_lenth_histogram_min_max_{method_name}_{perplexity}_{dataset}_{method}")
    # ### plot_histogram_values(max_rel_edge_ratio_per_row, bins= 30, output_path=output_path)
    ...
    # ###____________ABove  needs to be uncomment below 2 point________________________________
    ...
    # output_path = os.path.join(output_folder, f"Absolute_difference__total_proces_distort_{method_name}_{perplexity}_{dataset}_{method}")
    ...
    # output_path = os.path.join(output_folder, f'grids_projection_{perplexity}_{dataset}_{method}.html')
    ...
    ###________________________Kendall Tau rank distance_____________________________________________________________
    print('Run Kandall Tau ...')
    edges = list(itertools.combinations(range(len(low_dm_emb)), 2))
    edge_weights_hd = np.array([np.linalg.norm(D[i] - D[j]) for i, j in edges])  # Compute edge lengths once
    edge_weights_ld = np.array([np.linalg.norm(low_dm_emb[i] - low_dm_emb[j]) for i, j in edges])  # Compute edge lengths once

    # Sort edges from shortest to longest in HD
    sorted_indices_hd = np.argsort(edge_weights_hd)  
    # sorted_edges_hd = [edges[i] for i in sorted_indices_hd]
    # sorted_edge_weights_hd = edge_weights_hd[sorted_indices_hd]

    # Sort edges from shortest to longest in HD
    sorted_indices_ld = np.argsort(edge_weights_ld)  
    # sorted_edges_ld = [edges[i] for i in sorted_indices_ld]
    # sorted_edge_weights_ld = edge_weights_ld[sorted_indices_ld]

    hd_to_ld_position = [np.where(sorted_indices_ld == val)[0] for val in sorted_indices_hd]

    # kandel_tau =  normalised_kendall_tau_distance(sorted_indices_hd, hd_to_ld_position)

    kandel_tau, p_value = kendalltau(sorted_indices_ld ,hd_to_ld_position)
    kandel_tau_2, p_value_2 = kendalltau(hd_to_ld_position, sorted_indices_ld)
    
    # print(f'Kandal Tau :'{kandel_tau})
    print(f'Kandal Tau :{kandel_tau}')
    # breakpoint()

    ### _________________________END Kendall Tau rank distance________________________________________________________________
...
# output_path = os.path.join(output_folder, f"projection_inv_proj_metric_differnece_{dataset}_{method}")
# plot_proj_inverse_proj_metric_differences(prj_inverse_proj_diff, output_path)

# output_path = os.path.join(output_folder, f"projection_quality_metrics_heatmap_{dataset}_{method}")
# plot_metrics_perplexity_heatmap(prj_metrics_hd_to_ld.metrics, perplexities,output_path, "HD to LD")

# output_path = os.path.join(output_folder, f"projection_Inverse_quality_metrics_heatmap_{dataset}_{method}")
# plot_metrics_perplexity_heatmap(prj_metrics_hd_to_hd.metrics, perplexities,output_path, "HD to HD")

# output_path = os.path.join(output_folder, f"projection_HD_LD_metrics_vs_rate_change_perplexity_heatmap_{dataset}_{method}")
# plot_metrics_vs_rate_change_perplexity_heatmap(prj_metrics_hd_to_ld.metrics, perplexities,output_path, "HD to LD")

# output_path = os.path.join(output_folder, f"projection_HD_HDmetrics_vs_rate_change_perplexity_heatmap_{dataset}_{method}")
# plot_metrics_vs_rate_change_perplexity_heatmap(prj_metrics_hd_to_hd.metrics, perplexities,output_path, "HD to HD")

# output_path = os.path.join(output_folder, f"projection_HD_LD_metrics_vs_rate_change_combined_heatmap_{dataset}_{method}")
# plot_metrics_perplexity_with_combined(prj_metrics_hd_to_ld.metrics, perplexities,output_path, "HD to LD")

# output_path = os.path.join(output_folder, f"projection_HD_HD_metrics_vs_rate_change_combined_heatmap_{dataset}_{method}")
